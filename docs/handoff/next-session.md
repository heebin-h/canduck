...
- ⏸ **`w0/t02d-handoff-update` 브랜치 미push** — 이번 세션 전체 산출물(핸드오프·로그·BOM 확정·보고서 4종) 포함. heebin이 push + PR(→main) merge. (구 t02c는 이 브랜치로 rebase 통합됨.)
- ⏸ **BOM 발주 결제** — 2026-07-12 turn-8~10에서 최종 confirm 완료 (RPi4 보유분 사용, 전량 JLC 어셈블리, 배터리·스코프 등 보류, 랩 도구 최소 셋 — `hardware/parts.md` 참고, 품목 심사 후 코어 ~404K). 배치 1(~249K — A 확정 97K + 모터 52K + 3-in-1 카메라 ~100K) 결제 실행만 남음 (heebin, 이번 주 내 권장).
...
- ⚠️ **W1 설계 변경 (2026-07-12 품목 심사 반영)**: ① LCD·INMP441·PAM8403·스피커 탈락 → **Board B에서 I2S 라인, 오디오 앰프 회로, J_B3(LCD)/J_B4(마이크)/J_B6(스피커) 커넥터 삭제 검토** — 오디오·영상은 3-in-1 USB 카메라(마이크·스피커 내장)로 대체, PCB 무관. ② v1 표정 표시 = **tkinter 창**으로 확정 (2026-07-12 turn-18, `host/canduck/display_tk.py`) — RPi에 모니터 연결 또는 데스크탑 창. LCD 복귀는 display 백엔드 추가로 언제든 가능. ③ 모터 최종안은 `docs/reports/W0-T02g-motor-selection.md` (MG90S×10 통일 + N20 100RPM×2).
